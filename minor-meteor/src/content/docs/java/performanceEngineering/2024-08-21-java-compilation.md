---
title: JVM Architecture and Java Compilation Process
...
In Java, the compilation process involves 6 stages to transform Java
source code into bytecode that can be executed by the Java Virtual Machine (JVM).
...
Compilation => Source Code 
  ...
  ↓  → 5.Code Optimization 
  ↓  → 6.Code Generation → Bytecode (.class files)
  ↓
.class file (Bytecode)
  ↓
  ↓
